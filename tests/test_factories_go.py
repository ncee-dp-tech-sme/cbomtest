"""
tests/test_factories_go.py

Snippet factory tests (SF-01..05) for Go weakness factories.
...
# Build a list of (index, weakness, factory_fn) for parametrize IDs
_FACTORIES = [
    (i, wk, fn)
    for i, (wk, fn) in enumerate(gva.GO_WEAKNESS_FACTORIES)
]
_IDS = [f"go[{i}]-{wk.tag}-{wk.description[:40]}" for i, wk, fn in _FACTORIES]
