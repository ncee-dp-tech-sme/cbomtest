"""
tests/test_factories_dart.py

Snippet factory tests (SF-01..05) for Dart weakness factories.
...
_FACTORIES = [
    (i, wk, fn)
    for i, (wk, fn) in enumerate(gva.DART_WEAKNESS_FACTORIES)
]
_IDS = [f"dart[{i}]-{wk.tag}-{wk.description[:40]}" for i, wk, fn in _FACTORIES]
...
# Note: no Dart PQC factories currently; all parametrized cases will be skipped.
@pytest.mark.parametrize("idx,wk,fn", _FACTORIES, ids=_IDS)
