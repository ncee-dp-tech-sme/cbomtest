"""
tests/test_factories_c.py

Snippet factory tests (SF-01..05) for C/C++ weakness factories.
...
_FACTORIES = [
    (i, wk, fn)
    for i, (wk, fn) in enumerate(gva.C_WEAKNESS_FACTORIES)
]
_IDS = [f"c[{i}]-{wk.tag}-{wk.description[:40]}" for i, wk, fn in _FACTORIES]
...
# Note: no C/C++ PQC factories currently; all parametrized cases will be skipped.
@pytest.mark.parametrize("idx,wk,fn", _FACTORIES, ids=_IDS)
